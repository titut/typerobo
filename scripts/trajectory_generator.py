...
class TrapezoidVelocity:
    ...
    def __init__(self, trajgen):
        ...
        self.desired_v = trajgen.desired_v
        if self.desired_v is None:
            self.desired_v = [None] * trajgen.ndof
            for i in range(trajgen.ndof):
                self.desired_v[i] = (
                    1.25 * (self.final_pos[i] - self.start_pos[i]) / trajgen.T
                )
        self.T = trajgen.T
        self.ndof = trajgen.ndof
        self.X = [None] * self.ndof
    ...
    def solve(self):
        t0, tf = 0, self.T
        tb = [None] * self.ndof
        alpha = [None] * self.ndof
        for i in range(self.ndof):
            # calculate acceleration time
            tb[i] = (
                self.start_pos[i] - self.final_pos[i] + self.desired_v[i] * tf
            ) / self.desired_v[i]
            alpha[i] = self.desired_v[i] / tb[i]
    ...
    def generate(self, nsteps=100):
        ...
        for i in range(self.ndof):  # iterate through all DOFs
            q, qd, qdd = [], [], []
            tb = self.coeff[0][i]
            alpha = self.coeff[1][i]
            for t in self.t:  # iterate through time, t
                if t < tb:
                    q.append(self.start_pos[i] + (alpha * t**2) / 2)
                    qd.append(alpha * t)
                    qdd.append(alpha)
                elif t > tb and t < self.T - tb:
                    q.append(
                        (
                            self.start_pos[i]
                            + self.final_pos[i]
                            - self.desired_v[i] * self.T
                        )
                        / 2
                        + self.desired_v[i] * t
                    )
                    qd.append(self.desired_v[i])
                    qdd.append(0)
                else:
                    q.append(
                        self.final_pos[i]
