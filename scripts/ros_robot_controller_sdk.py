...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(control function realized through interface)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制(LED control)
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servo control, the board are numbered 1-4 from inside to outside)
    PACKET_FUNC_BUS_SERVO = 5  # 总线舵机控制(bus servo control)
    PACKET_FUNC_KEY = 6  # 按键获取(button obtain)
    PACKET_FUNC_IMU = 7  # IMU获取(IMU obtain)
    PACKET_FUNC_GAMEPAD = 8  # 手柄获取(handle obtain)
    PACKET_FUNC_SBUS = 9  # 航模遥控获取(model aircraft remote control)
...
class PacketReportKeyEvents(enum.IntEnum):
    # 按键的不同状态(the different states of button)
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(sub-command 0x01 set SSID, the second byte is the string length, this length includes the '\0' string terminator)
    ...
    def bus_servo_set_position(self, duration, positions):
        #print(f"setting pulse for {[duration, positions]}")
        duration = int(duration * 1000)
...
def bus_servo_test(board):
    ...
    time.sleep(4)
    print('2nd move \n')
    board.bus_servo_set_position(2, [[1, 200], [2, 200]])
    time.sleep(4)
    print('3rd move \n')
    board.bus_servo_set_position(3, [[1, 800], [2, 800]])
    time.sleep(4)
    print('4th move \n')
    board.bus_servo_set_position(4, [[1, 500], [2, 500]])
    time.sleep(4)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            positions = []
            for i in range(6):
                val = board.bus_servo_read_position(i)
